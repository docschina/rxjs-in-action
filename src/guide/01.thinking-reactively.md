...
确实，时间是不可或缺的影响因素。异步代码中最麻烦的部分就是处理延迟和等待时间。你看早期
...
RxJS -> Consumer: One programming model to handle sequences of events separated in time, as well as data stored in memory
