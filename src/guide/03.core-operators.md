...
const source$ = Rx.Observable.create(observer => {
  let i = 0;
  setInterval(() => {
    observer.next(i++); // This interval will keep on emitting events every 500 milliseconds until something stops it.
}, 500); });
```
...
...moments later
subscription.unsubscribe(); // Tears down the stream and frees up subscription.unsubscribe();
```
...
const progressBar$ = Rx.Observable.create(observer => {
  ...
  return () => { // Function that executes when the unsubscribe method is called. Describes how to cancel that timeout upon disposal.
