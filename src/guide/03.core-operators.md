...
这就是为什么我们需要像 RxJS 这样的久经考验的库。在 RxJS 中，由生产者负责取消订阅。订阅关系(subscription)的管理，是通过调用 subscribe() 后返回的一个类型为 Subscription（在 RxJS 4 中也称为 Disposable）的对象来处理的，此 subscription 对象实现了处理源流(source stream)的机制。如果我们已经结束观察，并且不想再接收事件，我们可以调用 unsubscribe() 来拆除卸下它（指源流）；这被称为显式地取消订阅(explicit cancellation)。下面是一个简化的示例：
...
回想一下，在第 2 章中，我们介绍了 Rx.Observable.create() 方法，此方法可用于创建任意的可观察对象。创建它的最后一步是，指出订阅后返回的 subscription 对象该如何处理可观察对象，这是你必须实现的行为。回到我们的进度指示器代码，像这样，在最后添加取消订阅(unsubscription)机制。
